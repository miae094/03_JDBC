...
public class JDBCExample1 {
	public static void main(String[] args) {
		...
		try {
			...
			String host = "localhost"; // DB 서버 컴퓨터의 IP 혹은 도메인주소
										// localhost : 현재 컴퓨터
			...
			// oracle.jdbc.driver.T4CConnection@6973bf95
			...
			/* 4. Statement 객체 생성 */
			stmt = conn.createStatement(); // Java와 DB를 연결하는 통로를 다니는 버스
			// 연결된 DB에 sql을 전달하고 결과를 반환받을 역할을 할
			// Statement 객체를 생성해줌
			
			/* 5.Statement 객체를 이용해서 SQL 수행 후 결과 받환받기 */
			// 1) ResultSet Statement.executeQuery(sql)
			// -> sql 이 SELECT문일때 결과로 Result 객체 반환
			
			// 2) int Statement.executeUpdate(sql);
			// -> sql 이 DML(INSERT, UPDATE, DELETE) 일때 실행 메서드
